...
function generateGroupSystemPrompt(group) {
    ...
    prompt += `   - \`[${group.me.nickname}的语音：...]\`, \`[${group.me.nickname}发来的照片：...]\`（「照片」与冒号之间可写 ·锁脸 / ·空镜 / ·局部，与生图设置一致）: 我发送了特殊类型的消息，群成员可以对此发表评论。\n`;
...
- **普通消息**: \`[{成员真名}的消息：{消息内容}]\``;
    ...
    outputFormats += `
- **语音**: \`[{成员真名}的语音：{语音转述的文字}]\` —— **语音转述的文字** 须为纯口语台词（可含标点与拟声字）；**禁止**用括号或星号写表演说明、笑声、气声、神态、旁白等（与私聊语音规则一致）。
- **照片（配图）**: \`[{成员真名}发来的照片：{内容描述}]\`（可选用 \`发来的照片·锁脸\` / \`·空镜\` / \`·局部\` 再接冒号，含义同私聊生图说明；系统仅为静态图，无单独「视频」消息类型）
...
- **结束私聊**: \`[Private-End: {发起者真名} -> {接收者真名}]\``;
   ...
   if (allWorldBookContent.includes('<orange>')) {
    ...
    prompt += `   - **内容多样性**: 你的回复应以普通文本消息为主，但可以 **偶尔、选择性地** 让某个成员发送一条特殊消息（语音、照片/配图），以增加真实感。不要滥用特殊消息。\n`;
