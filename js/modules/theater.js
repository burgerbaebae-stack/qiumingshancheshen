...
const TheaterMode = (() => {
    ...
    function looksLikeTheaterBlocksJson(s) {
        const t = String(s || '');
        return /"blocks"\s*:/.test(t) && /\[\s*\{/.test(t) && /"type"\s*:/.test(t);
    }
    ...
    /** 读取 JSON 字符串内容（从开引号后的第一个字符起），若 leaked 则返回 leaked:true 与截至 lim 的片段 */
    ...
    function decodeJsonQuotedChunk(chunk) {
        if (!chunk) return '';
        return chunk
            .replace(/\\n/g, '\n')
            .replace(/\\r/g, '\r')
    ...
    /** 未闭合整体 JSON 时尽力提取 scenePrompt / sessionState 的字符串值 */
    function tryExtractTheaterJsonStringField(raw, key) {
        const re = new RegExp(`"${key}"\\s*:\\s*"((?:[^"\\\\]|\\\\.)*)"`, 'm');
        const m = re.exec(String(raw || ''));
        if (!m) return '';
        return decodeJsonQuotedChunk(m[1]);
    }

    /** 将模型返回的 type 别名统一为 narration / dialogue（salvage 依赖，须在 salvage 之前声明） */
    ...
    const THEATER_SALVAGE_MAX_SCAN = 400000;
    const THEATER_SALVAGE_BLOCK_HEAD = 100000;

    /** JSON 毁坏（text 未闭引号等）时从 raw 中抢救 blocks；无外部依赖 */
    function salvageTheaterBlocksFromRaw(s) {
        const full = String(s || '').replace(/\r\n/g, '\n');
        const arrStart = findTheaterBlocksArrayContentStart(full);
        if (arrStart < 0) return [];

        let p = arrStart;
        const lim = Math.min(full.length, arrStart + THEATER_SALVAGE_MAX_SCAN);
        const out = [];

        while (p < lim) {
            while (p < lim && /[\s,\n\r]/.test(full[p])) p++;
            if (p < lim && full[p] === ']') break;
            if (p >= lim || full[p] !== '{') break;

            const objStart = p;
            const headEnd = Math.min(objStart + THEATER_SALVAGE_BLOCK_HEAD, lim);
            const head = full.slice(objStart, headEnd);

            const typeM = /"type"\s*:\s*"((?:[^"\\]|\\.)*)"/.exec(head);
            const typeRaw = typeM ? decodeJsonQuotedChunk(typeM[1]) : 'narration';

            const spM = /"speaker"\s*:\s*"((?:[^"\\]|\\.)*)"/.exec(head);
            const speakerRaw = spM ? decodeJsonQuotedChunk(spM[1]) : '';
            ...
            /** 裸 " / 烂尾时 readJson 会过早闭串；若下一非空白不是 } 或 ,，回退为漏串截取以尽可能保留 raw 内正文 */
            ...
            if (!useLeakedPath) {
                ...
                if (full[endBound] === ']') {
                    ...
                    if (commaBrace < 0) break;
                    next = commaBrace;
                }
            }

            if (textBody) {
                const ty = coerceTheaterBlockType(typeRaw) === 'dialogue' ? 'dialogue' : 'narration';
                out.push({
                    type: ty,
                    speaker: speakerRaw,
                    text: textBody
...
3. 【分块｜必读】前端按 JSON 里 blocks 的每一条展示一屏，用户点一次「继续」进下一条。禁止把多句旁白或多句对白塞进同一个 "text" 里。
   ...
   · **JSON 合法性**：每个 \`"text"\` 的字符串值内禁止未转义的原始换行（换行须写成 \`\\n\` 或拆成更多条 narration）；\`"text": "…"\` 的闭引号 \`"\` 必须在正文结束后立即出现，不得把旁白续写在引号外。**字符串值内若需要引号，须写成 \`\\"\` 或改用中文弯引号「」；禁止在段落中间插入裸的 ASCII \`"\`，否则整份 JSON 会断裂、后文全部丢失。**
    ...
    function parseTheaterJson(raw) {
        ...
        if (!obj || typeof obj !== 'object' || Array.isArray(obj)) {
            if (looksLikeTheaterBlocksJson(text)) {
                const salvaged = salvageTheaterBlocksFromRaw(text);
                if (salvaged.length) {
                    return {
                        blocks: salvaged,
