...
function scheduleDayBuildGenerationUserPrompt(char) {
    ...
    parts.push(
        `【模式 A · 纯便签】为角色「${char.realName || char.name}」填写**日历日 ${today}** 的**日程便签**（不是日记、不是小说段落、不写心理与对话）。\n` +
            ...
            `与对方（${char.myName}）有关的 **具象事务**（见面、送礼、代办事、约定到访等）计划行 **不宜超过 2 行或占总行数约 35%**（取更严者；整日无此类事务则可 0 行；**对话已约定之会面/连线若与当段主事合并为一行，按一行计**）。\n` +
            `【禁止】段落体；「他感到/心想/冷声/轻笑」等描写；引号对话；比喻与排比；单条内出现「并/因此/随后」串联多件以上的长句。`
    );
    parts.push(
        `\n【硬性要求】\n` +
            `**总序**：便签以**当前剧情与对话约定**为先；下列粒度规则用于**防灌水**，**禁止**用其回避、抹掉聊天记录或待办中**已明确约定**的时段与事项（否则会与人设「失忆」般矛盾）。\n` +
