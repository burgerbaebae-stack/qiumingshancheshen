...
function _buildPrivateReplyCountBlock(character) {
    if (!character || !character.replyCountEnabled) return '';
    ...
    return (
        ...
        `此约束**优先于**本节下文第 3、4 条中「可一条收住」「允许整轮只有一两条」等宽松表述；仍须遵守第 2 条「一句一条」、禁止把多句硬塞进同一条可见消息；**logic_rules**（如第 10 条须用 **i)** 引用）及双语等特殊规则的优先级仍更高。若需凑足条数，可用多条简短自然的 a) 承接，避免空洞复读。\n\n`
    );
}
...
p) 求代付: [${character.realName}向${character.myName}发起了代付请求:金额|商品清单]`;
   ...
   if (allWorldBookContent.includes('<orange>')) {
    ...
    if (character.replyCountEnabled) {
        prompt += `**若已开启用户自定义条数，单轮可见消息条数以该节内【用户设置·回复条数】为准。**`;
    }
    prompt += `\n`;
