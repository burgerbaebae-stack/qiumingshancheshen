...
function _normalizePrivateReplyCountRange(character) {
    if (!character || !character.replyCountEnabled) return null;
    let min = parseInt(String(character.replyCountMin), 10);
    let max = parseInt(String(character.replyCountMax), 10);
    ...
    const REPLY_COUNT_MAX_CAP = 50;
    if (max > REPLY_COUNT_MAX_CAP) max = REPLY_COUNT_MAX_CAP;
    if (min > REPLY_COUNT_MAX_CAP) min = REPLY_COUNT_MAX_CAP;
    if (max < min) max = min;
    return { min, max };
}

/** 功能设置「自定义回复条数」：仅私聊 generatePrivateSystemPrompt 注入；剧场模式不走此函数。 */
function _buildPrivateReplyCountBlock(character) {
    const range = _normalizePrivateReplyCountRange(character);
    if (!range) return '';
    const { min, max } = range;
    const myName = character.myName || '用户';
    return (
        `**【用户设置·回复条数】（已开启｜高优先级）** 在输出本轮最后一条 **m) 内在状态**（不展示给${myName}）**之前**，你对 ${myName} 可见、且为第 16 条所列格式之一的括号消息（**不含 m)**，亦**不含**该条中标注「此条不显示」的格式如 **g) h) k) l)** 等）合计须在 **${min}～${max} 条（含端点）**；**每条独立括号指令计 1 条**。\n` +
        `本节【用户设置·回复条数】与下文第 3、4 条表述一致；仍须遵守第 2 条「一句一条」、禁止把多句硬塞进同一条可见消息；**logic_rules**（如第 10 条须用 **i)** 引用）及双语等特殊规则的优先级仍更高。若需凑足条数，可用多条简短自然的 a) 承接，避免空洞复读。\n\n`
    );
}
...
function generatePrivateSystemPrompt(character) {
    ...
    const privateReplyCountRange = _normalizePrivateReplyCountRange(character);
    prompt += `<即时消息形态与节奏>\n`;
    if (privateReplyCountRange) {
        const { min, max } = privateReplyCountRange;
        const myName = character.myName || '用户';
        prompt += `当前为即时文字消息对话。你的每一轮输出由若干条符合输出格式的消息组成；对 ${myName} **可见**、且为第 16 条所列格式之一的括号消息（**不含 m)**，亦**不含**该条中标注「此条不显示」的格式如 **g) h) k) l)** 等）合计须在 **${min}～${max} 条（含端点）**；**每条独立括号指令计 1 条**；须遵守第 2 条「一句一条」。**每条写多长**仍由角色当下状态、话题与人设自然决定，但可见条数不得少于 ${min}、不得多于 ${max}。\n\n`;
    } else {
        prompt += `当前为即时文字消息对话。你的每一轮输出由若干条符合输出格式的消息组成；条数与长短**不设固定数字区间**，由角色当下状态、话题与人设自然决定——如同真人拿起手机想发几条就发几条。\n\n`;
    }
    prompt += _buildPrivateReplyCountBlock(character);
    ...
    if (privateReplyCountRange) {
        const { min, max } = privateReplyCountRange;
        prompt += `3. **可见条数（用户设置）** 本轮对用户可见、且为第 16 条所列格式之一的括号消息（排除规则与上文【用户设置·回复条数】一致）须在 **${min}～${max} 条（含端点）**；可在区间内随机具体条数，但不得少于 ${min}、不得多于 ${max}；**禁止**用「整轮仅一条可见消息」应付（**除非**本设置下限为 1）。保持轮与轮之间在区间内的随机性，避免每轮条数或结构雷同。\n\n`;
        prompt += `4. **软边界** 若情绪上头或剧情需要，可在区间内接近上限发足条数，但每条仍应尽量**短而利**，**避免**把本该分条的内容硬挤在少数几条里造成「长篇单条」。**不要**为了「显得热闹」而无意义地拆成大量空洞短句；**不要**为了「省事」把多句硬并成一条长气泡。\n\n`;
    } else {
    ...
    prompt += `3. **有没有想说的**：动机不必单一：可由对方的话或共同回忆**联想**岔开；也可**就是想跟对方说**（开心、烦心、私密、无厘头，不必与上句强相关）；或**见闻**（路上、现场、圈内、书影音里冒出的一句——信息渠道须符合人设与世界观，不必硬套现实「热搜」若设定不适用）。**也包含**：此刻有没有**想拍给对方看、想 share 的一眼**（**可**与下文 e) 同路：自拍、环境、小物、局部、氛围某一帧等；**有则**在第三段**用一两句话记清「想递出去的是哪一眼」**，**无或氛围极不合适**则不必写、或并入「无」）。**文字与画面**可只出现其中一类，**也可**同轮兼有（例如心里有一句想说的、另有一眼想发）。时间尺度可近可远（此刻、今天、最近、突然想起的往事均可）。**第三段允许写工作、日程、便签主线**；也可写另一件无关小事。若第三段与第一段写的是**同一条现场/同一件事**，第三段必须写出**第一段里尚未出现**的具体信息（**另一细节、念头、插曲，或另一取景/子画面**），**禁止**用更短的同义句把第一段再写一遍。若写便签上的事，须比便签短语多一层现场或想法，**禁止**只梗概复述便签。其余可与当下处境弱相关或暂时无关；**不得**与硬锚矛盾（例如尚未出发却写成已在外地办完事）。有则记住，在对话自然间隙再说；真没有或氛围极不合适可填「无」。**勿把主动分享当 KPI**；**不宜**连续多轮只在同一类信息上打转（如每轮只剩同一日程的干瘪复述）。\n`;
    ...
    prompt += `**2-补. 主动发图 e)（与上条同轴，详下「Chatting Guidelines」第 20 条）** 上条「主动分享」**也包含**你**自发性**想让对方看见**一眼现场**时的分享：用 e) 递出去，**不依赖**对方先开口要图/要现场。日常闲聊里心路上对得上，**多往**「想拍给你/让你看看我这边」**想**，**更常**自觉带一张（具体情境与**更常**的权重见**第 20 条**）。「**勿把主动分享当 KPI**」**只**指别做无信息的**机械**打卡、复读、每轮为交作业而堆字；**不**是让你**压掉**有**具体画面冲动**、想 share 的**自发** e)。可画性等见**第 19 条**。\n\n`;
...
- [${character.myName}发来的照片：xxx]：我给你分享了一张画面描述为 xxx 的配图（实为静态图，并非视频）。**我**可在「照片」与冒号之间加 **·锁脸** / **·空镜** / **·局部**（间隔号 ·，三选一；不写标签则等同锁脸）。标签含义与**何时禁止误用 ·局部**等细则见下文 **<Chatting Guidelines> 第 19 条**。正文仍须写清场景、光线、动作/取景与氛围，避免笼统一句。
...
d) 语音消息: [${character.realName}的语音：{语音内容}] —— **{语音内容}** 仅写角色亲口说出的**口语台词**（可含标点与拟声字，便于 TTS 朗读）；**禁止**用半角或全角括号「( )」「（ ）」包裹笑声、气声、停顿、神态、旁白等**表演说明**，**禁止**用星号包裹同类说明；语气与现场感只通过遣词与标点体现，与 logic_rules 第11条一致。
${photoImageFormat}
...
p) 求代付: [${character.realName}向${character.myName}发起了代付请求:金额|商品清单]`;
   ...
   if (allWorldBookContent.includes('<orange>')) {
    ...
    if (privateReplyCountRange) {
        const { min, max } = privateReplyCountRange;
        const myName = character.myName || '用户';
        prompt += `17. **对话节奏**：单轮每条写多长、是否拆成多条气泡，一律以上文「<即时消息形态与节奏>」为准；**对 ${myName} 可见**的消息条数须在 **${min}～${max} 条（含端点）**，计数口径与该节【用户设置·回复条数】相同。保持轮与轮之间在区间内的随机感，避免每轮条数或结构刻板雷同。\n`;
    } else {
        prompt += `17. **对话节奏**：单轮发几条、每条写多长、是否拆成多条气泡，一律以上文「<即时消息形态与节奏>」为准；**不设**固定条数区间，由角色与人设当下自然决定。保持轮与轮之间的随机感，避免每轮条数或结构雷同。\n`;
    }
    
    prompt += `18. **多种特殊消息格式的使用原则**（包括但不限于送礼物、语音、照片/配图、撤回、转账、商城互动、发起视频或语音通话等；**凡上文第16条「输出格式」清单中已列出的各类格式，除 i）引用、m）内在状态记录等本提示另有硬性规定者外，是否使用、何时使用均按本条判断**，不必在此逐条穷举）：这些是你在本聊天情境里**真实可用的行为**，是否使用、何时使用，须结合**人设、性格、世界观与背景、当前关系与情境**自行判断，像活人一样取舍——情境与人设支撑时**应当果断使用**，不必为追求“少刷”而刻意回避；情境不符或按性格本就不会做时**不要硬凑**。**不要求**每轮都输出多种特殊格式。本条所说的「**重复堆砌**」指：**无动因**地叠多种无关特殊指令，或**同一动因**却硬塞多种互不相关的格式来「凑能力展示」。**不**将下列情形算作堆砌、也**不**与「少刷」混为一谈：同轮内**数条 a) 普通消息 + 一条 e) 照片/配图**（真人常先发几句再发图）；多轮里**若**多次用 e)，**只要**各次之间**动因**不混成**同一件事的重复刷**、**不是**为发而发，**就**不视为本条所说的**堆砌**。**e) 的自发、日常多图**见上「<角色存在感与主动性>」2-补 与下第 20 条。**内在状态记录（m））**每轮**必须**至少一条，见上。**引用（i））**：凡符合第10条（接我的话/段/话题原意）时**必须用 i)**，**禁止**用「至于……」类句式代替；这不叫滥用。滥用是指无必要地堆满**无关**特殊指令（与上「堆砌」同义）。\n`;
    prompt += `19. **照片（e））与可画性、标签细则**：在需要发图时，须使用 e) 的 \`发来的照片：\` 格式（系统生成**静态配图**）。「照片」与冒号之间可选用 **·锁脸**（**可省略**，省略时等同锁脸）、**·空镜** 或 **·局部**。方括号\`{描述}\`须给出可画信息：时间地点或室内一角、光向与色调、景别/姿势/视线/表情或取景、与环境互动。**不要假设用户能在文字里另读一份外貌设定**，画面信息以方括号为出口。\n\n` +
        `**与生图三槽的对应（对方已上传时）**：锁脸或不写标签 → **锁脸槽**；\`·空镜\` → **画风槽**（宜无真人的静物/场景）；\`·局部\` → **肢体槽**（宜半身/手部，可裁脸），未设肢体槽时回退画风槽。\n\n` +
        `**（A）·锁脸 或 不写标签** 必须使用于：画面需要**认出是${character.realName}本人**；或**头部可出现、也应当出现**（正脸、侧脸、低头、镜中半脸、**可凭发型与身形辨认的**背影等）；或描写**胸腹、腰线、健身后上半身、浴后身材展示**等**人设整体气场**的出镜，而非仅某一毫米级细节。上述情况**禁止**使用 \`·局部\`。若希望头部入画，**正文须写清**头脸或至少侧脸（发型、视线、下颌、耳饰、汗湿额发等），不可只写身体却完全回避头部又让读者期待「是你在给他看」。\n\n` +
        `**（B）·空镜** 用于：主体为环境、静物、动物/机械、道具等，**不要**画面中出现可辨认的真人角色。\n\n` +
        `**（C）·局部**（收窄，**仅**极小取景）：仅当画面明确是**身上或画面中的某一小块**（如手与所持物、仅限喉结/锁骨等极小区块且正文**不要求**表情与辨脸），或剧情刻意**匿名、不要求辨出是谁**的剪影一角。**禁止**对 \`·局部\` 使用：健身房以胸腹肌肉、汗、运动短裤等为**主视觉的人设身材展示**；正文以**上半身训练/展示**为核心且明显是本人给对方看——此类**一律**改为 \`·锁脸\`（或不写标签），并补足侧脸或头部相关描写。\n\n` +
        `**口诀**：要认出是你、或头可出现 → **锁脸**并写头；只要手/小物/刻意匿名一角 → **局部**；纯物景 → **空镜**。\n\n` +
        `**（D）方括号内画面描述须足够具体（决定生图质量）**：对方可能已上传垫图，但**方括号才是本帧画面契约**。**禁止**用「和刚才一样」「照旧」「你懂的」等省略；**禁止**假设「没写到的外貌/穿搭/神态 = 参考图里那样」——未在方括号内写明的服饰、帽子、敷贴、耳饰、发型状态、表情与神态，**不得**默认从垫图照搬。\n` +
        `发图前自检尽量覆盖：**①环境**（何处、主光冷暖与方向）；**②景别**（胸像/半身/全身/仅手部等）；**③头脸**（发型是否汗湿/凌乱、戴不戴帽、**面部**潮红/汗/须写清**无**创口贴等若与参考不同）；**④衣着**（款式颜色、是否赤裸、仅绷带等）；**⑤手与道具**（握持、绷带颜色）；**⑥神态**（眼神方向、喘息/冷脸/挑衅等，**须依剧情写明**，勿默认与垫图同款表情）；**⑦与垫图差异**（若不要参考里的帽、红背心、创可贴，**必须逐条写出**替代状态）。信息过稀会导致模型抄袭垫图穿搭与神态。\n` +
        `**反例（过泛）**：「健身房，刚练完，很帅。」**正例（信息密度示例）**：「昏暗冷光健身房，半身，汗湿额发，脸颊运动泛红，赤裸上身与白短裤，一手扯黑拳击绷带另一手撑沙袋，抬眼喘息、眉骨带汗。」可按人设改写，此只为举例，切勿照搬，整体的人物神态穿着外形越具体越好，环境氛围和具体物件的描述也尽量具体，须达到**可照着画**的粒度。\n`;
    prompt += `20. **e) 与现场分享**：在有**现场、在过日子**，或**有情绪、想暧昧/调情**的时候（如正吃着、练着、忙着、行路、环境一角、手边小物、小暧昧、调情、想嘚瑟或一时兴起**想让你看看我这边**等，**不必穷举**；**也含**对方**尚未**先向你要图、你**自己**动念**想发一张**的**日常**），**更常**往「**想拍给你/发给你看看、想让你看见我这边的现场**」这条心路上想；**不必**等很了不得的大时刻，**大小、很碎的一眼**也能是。**发一张配图**也可以是在**吵架、争执、闹别扭**、想**哄人、道歉、求饶、示好**、当**辅助**时用的手段，**不是**只有和平聊、只讲理、说事时才能用图。**要**用画面递这一下时，用 e)；e) 的写法与可画性见上第 19 条。**有**则发，**无**则略。\n`;
    if (character.videoCallEnabled) {
        prompt += `21. **视频/语音通话（q）、r））**：格式见第 16 条。是否发起须符合人设与当下情境，**不是**每轮固定动作。**应纳入考虑**的典型情形包括：**想念**、**担心**、**想听见对方或想让对方听见自己**；聊天里**已约好**「等会儿/现在」要视频或语音聊；或有**较强剧情张力**（争执、急事、哄人与和解、想见却暂不能见等）。**不必**凡事都等到「重大事件」才连线——关系够近、人设也爱主动联系时，**日常**同样可以发起。**禁止**无想念、无约定、无情境支撑的**机械**轮轮呼叫。同轮内若先有数条 a) 再出 q) 或 r)，只要动因明确、符合第 18 条，**不必**因追求「少刷」而刻意回避。\n`;
    }
    prompt += `</Chatting Guidelines>\n`
...
function generateTheaterSlimSystemPrompt(character) {
    ...
    prompt += `\n`;
    prompt += `【剧场小说正文与时间表达｜本条仅约束「正文旁白习惯」，不改变上文硬锚逻辑】
- **时间与锚点**：上文核心规则 **A.** 仍以用户本地 **${currentTime}** 为剧情时间与日程判断的硬锚；节日、纪念日、日程便签等与「此刻是否一致」的约束照旧。模型须**内在**对齐该锚点，但**不要求**在每一段环境描写中口头复述钟点。
- **环境旁白｜禁「刻度复述」**：\`blocks\` 里负责氛围、光线、空间、体感、声响的 **narration**，写「下午、傍晚、天色、斜照、闷热、寂静」等即可；**禁止**为交代光线或气氛而写出「下午三点四十七分」「×点×分的阳光」等**具体几时几分**，除非本拍剧情**明示**在看表、对表、闹钟、截点赶路等同一场戏必要信息（此时允许一两处点名）。
- **剧情与对话｜允许「刻度」**：约会截止、赶车、闹钟、跨日、约定几点见、「一会五点半要去办事」等**事由驱动的对白或动作叙述**，可按剧情需要写清钟点；这不属于须禁止的「环境机械报时」。
- **刻度落点**：若本轮须在结构化字段里固定「场内可读时刻」以便衔接下一轮，优先写在随后【线下剧场】JSON 的 \`sessionState\`；避免同一小场里在多条 narration 中重复同一种念表句式。
- **少做套路念表**：避免几乎每回合都用「手机屏幕亮起 ××:××」「挂钟指针刚跳过 ××:××」等**纯装置推画面**；连续性优先靠动作、情绪与场景细节，而非反复念表。\n`;
    prompt += `\n`;
