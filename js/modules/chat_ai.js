...
- **时间锚照旧**：上文核心规则 **A.** 仍以用户本地 **${currentTime}** 为「故事里此刻」的唯一硬锚；节日、纪念日、日程便签、是否与本机时刻矛盾的约束**不改变**——你仍须在叙事内部对齐。
- **少做「刻度式环境报时」**：避免几乎每一回合都用「手机屏幕无声亮起，时间跳到 xx:xx」「挂钟指针刚好跨过 xx:xx」等来推画面；连续性不必靠反复念表维持。
- **刻度放哪**：与用户本地相容的**可读钟点或可理解时段**，优先写入随后【线下剧场】JSON 的 \`sessionState\`；\`blocks\` 旁白里用天色、光线、体感、困倦、炉火、人声远近等推进即可。
- **何时可在正文点名钟点**：约会截点、闹钟、跨日、赶路等剧情**强依赖**刻度时再写「几点几分」；同一小场不宜重复同一种「读表演化」报时。\n`;
    prompt += `\n`;
